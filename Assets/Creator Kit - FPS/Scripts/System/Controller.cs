...
public class Controller : MonoBehaviour
{
        ...
        if (!podeMover)
        {
            ...
            if (!m_IsPaused && !LockControl)
            {
                if (podePular)
                {
                    ...
                    if (podePular && m_Grounded && Input.GetButtonDown("Jump"))
                    {
                ...
                {
                    move = new Vector3(Input.GetAxis("Horizontal Invertido"), 0, Input.GetAxisRaw("Vertical Invertido"));
                }
    ...
    {
        if (hit.gameObject.tag == "Madeira")
        {
            FootstepPlayer.tipos = TiposDeChao.Madeira;
        }else if(hit.gameObject.tag == "Terra"){
            FootstepPlayer.tipos = TiposDeChao.Terra;
        }
        else
        {
            FootstepPlayer.tipos = TiposDeChao.Corredor;
        }
    }
